...
def main():
    ...
    # load files
    with open(path_dict['files'], 'r') as f:
    ...
    if args.snapshot:
        make_snapshot(
            allfiles,
            path_dict['golden_json'], path_dict['pu_json'],
            mets, list(hbins['pileup'].keys()),
            path_dict['snap_dir'],
            args.jobs,
    ...
    if args.hists:
        make_hists(path_dict['snap_dir'], path_dict['hist_dir'], hbins, args.jobs)

    # step 3: fit linear functions to 2d histograms
    if args.corr:
        get_corrections(path_dict['hist_dir'], hbins, path_dict['corr_dir'], path_dict['plot_dir'], mets, lumilabels)

    # closure
    if args.validate:
        make_validation_plots(path_dict['snap_dir'], path_dict['plot_dir'], path_dict['corr_dir'], hbins, axislabels, lumilabels)


if __name__=='__main__':
    main()
# TODO: discuss whether the choice of no filter introduces biases.
